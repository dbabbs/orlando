...
map.attributionControl.addAttribution(
  '<a href="https://https://tangrams.readthedocs.io/en/latest/">Tangram</a> | &copy; <a href="https://developer.here.com">HERE 2019</a>'
);
...
async function start() {
  const url = "./data/fire_stations.geojson";
  ...
  const jurisdictions = [
    ...new Set(data.features.map((x) => x.properties.jurisdiction)),
  ].sort();
  ...
  jurisdictions.forEach((j) => {
    ...
    div.innerHTML = `
      <label for="${j}">
         <input id="${j}" type="checkbox" name="jurisdiction" class="filter" ${
      j === "Orlando" ? "checked" : ""
    }>
         ${j}
      </label>`;
...
function countStations(stations) {
  stations.forEach(
    (s) => (counter[s] = counter[s] === undefined ? 1 : counter[s] + 1)
  );
}
...
async function refresh(from = "filter") {
  ...
  $("#num").innerHTML = `<span class="emphasis">${activeCount}</span> station${
    activeCount === 1 ? "" : "s"
  ...
  setMax(rangeType);
  setRangeText();
  const isolinePromises = coordinates.map((x) =>
    fetch(isolineUrl(x.coordinates, range, rangeType)).then((res) => res.json())
  );
  ...
  responses.forEach((x, i) => {
    const shape = x.response.isoline[0].component[0].shape.map((z) =>
      z.split(",")
    );
...
function formatTooltip({ jurisdiction, address, status, _latlngs, name }) {
  const geojsonPolygon = turf.lineToPolygon(
    turf.lineString(_latlngs[0].map((x) => [x.lng, x.lat]))
  );
  const areaMilesSquared = turf.area(geojsonPolygon) / 2.59e6;
  return `\
   <div>
      <h3>${name}</h3>
